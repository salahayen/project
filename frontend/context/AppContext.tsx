...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Initialize Mock Data (Auth related only for safety, others will be fetched)
    setClients(MOCK_CLIENTS);
    setExperts(MOCK_EXPERTS);
    setRequests(MOCK_REQUESTS);
    setAdmins(MOCK_ADMINS);
    // setServices(SERVICES); // Fetched from DB
    // setPlans(MOCK_PLANS);  // Fetched from DB
    ...
    const initBackend = async () => {
      try {
        // Check for Social Login Session (NextAuth)
        try {
          const sessionRes = await fetch(`${API_BASE_URL}/api/auth/session`);
          if (sessionRes.ok) {
            const session = await sessionRes.json();
            if (session.user && !user) {
              console.log('Hydrating user from Social Session:', session.user);
              setUser(session.user);
              localStorage.setItem('finume_user', JSON.stringify(session.user));
            }
          }
        } catch (e) {
          console.log('No active social session');
        }
        ...
        if (usersRes.ok) {
          ...
          // Merge Real DB Users with Mocks to keep the "Demo" populated feel
          setClients([...(usersData.clients || []), ...MOCK_CLIENTS]);
          setExperts([...(usersData.experts || []), ...MOCK_EXPERTS]);
          setAdmins([...(usersData.admins || []), ...MOCK_ADMINS]);
        ...
        try {
          ...
          if (servicesRes.ok) {
            ...
            if (Array.isArray(servicesData) && servicesData.length > 0) {
              ...
              console.warn('DB Services empty, using mocks');
              setServices(SERVICES);
            }
          }
        } catch (e) {
          console.error("Failed to fetch services", e);
          setServices(SERVICES); // Fallback
        }
        ...
        try {
          ...
          if (plansRes.ok) {
            ...
            if (Array.isArray(plansData) && plansData.length > 0) {
              ...
              console.warn('DB Plans empty, using mocks');
              setPlans(MOCK_PLANS);
            }
          }
        } catch (e) {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Always merge backend data with mocks
        // if (Array.isArray(data) && data.length > 0) { // <-- REMOVED condition to force merge even if empty
        setRequests(prev => {
          const newIds = new Set(Array.isArray(data) ? data.map((d: any) => d.id) : []);
          // FIX: Mocks start with 'REQ-', not 'R-'
          const filteredPrev = prev.filter(p => !newIds.has(p.id) && p.id.startsWith('REQ-'));
          return [...(Array.isArray(data) ? data : []), ...filteredPrev];
        });
        // }
  ...
  const updateRequestStatus = async (id: string, status: Request['status']) => {
    ...
    showToast(`Request status updated to ${statusText}`, 'success');
  ...
  const assignRequest = async (requestId: string, expertId: string) => {
  ...
  const updateExpertStatus = async (expertId: string, status: Expert['status']) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/users`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: expertId, status })
      });
    } catch (e) {
      console.error("Failed to update expert status", e);
    }
  };
  ...
  return (
    <AppContext.Provider value={{
      ...
      settings, updateSettings, getPermissions, updatePermissions, clientPermissions,
