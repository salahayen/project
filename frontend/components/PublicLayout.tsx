...
const PublicLayout = () => {
  ...
  const navLinks = [
    ...
    { name: language === 'ar' ? 'تدقيق الالتزام' : 'Compliance Audit', path: '/compliance-audit', visible: true },
              ...
              {user ? (
                <>
                  ...
                  <Button
                    ...
                    variant="danger"
                    onClick={logout}
                    className="bg-transparent hover:bg-red-50 text-red-600 border border-transparent hover:border-red-100"
                  >
                    {t('nav.logout') || 'Logout'}
                  </Button>
                </>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-20 left-0 w-full bg-white border-b border-gray-100 shadow-xl p-4 flex flex-col gap-4 animate-in slide-in-from-top-5 duration-200">
            ...
            <div className="border-t border-gray-100 my-2 pt-4 flex flex-col gap-3">
              {user ? (
                <>
                  <Link to={`/${user.role.toLowerCase()}`} onClick={() => setIsMobileMenuOpen(false)}>
                    <Button variant="outline" className="w-full justify-center">{t('nav.dashboard') || 'Dashboard'}</Button>
                  </Link>
                  <Button
                    variant="danger"
