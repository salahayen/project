...
const TierCalculator: React.FC = () => {
    ...
    const getRecommendation = () => {
        if (!plans || plans.length < 3) return { plan: null, reason: 'Loading...' };

        if (revenue > 5000000) return { plan: plans[2], reason: 'Revenue > 5M SAR requires statutory Financial Audit' };
        if (transactionTier === 2) return { plan: plans[2], reason: 'High volume transactions require enterprise bookkeeping' };
        if (employeeTier === 2) return { plan: plans[2], reason: 'Large team requires advanced payroll & HR support' };

        if (revenue > 375000) return { plan: plans[1], reason: 'Revenue > 375k SAR mandates full VAT compliance' };
        if (transactionTier === 1) return { plan: plans[1], reason: 'Active business requires monthly bookkeeping' };
        if (employeeTier === 1) return { plan: plans[1], reason: 'Employees require GOSI & Payroll management' };

        return { plan: plans[0], reason: 'Best for maintaining legal status with minimal activity' };
    };

    const { plan, reason } = getRecommendation();

    if (!plans || plans.length < 3 || !plan) return null;
                        ...
                        <div className="relative z-10 animate-in fade-in slide-in-from-right-4 duration-500" key={plan.id}>
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => {
                                        if (user) {
                                            navigate(`/client/checkout?planId=${plan.id}`);
                                        } else {
                                            navigate(`/login?redirect=/client/checkout&planId=${plan.id}`);
                                        }
                                    }}
